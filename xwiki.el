...
(defun xwiki-reload-tomcat ()
  (interactive)
  (shell-command "touch ~/projects/vendor/tomcat/apache-tomcat-9.0.14/webapps/ROOT/META-INF/context.xml")
  )
...
(defun publish-ext-collab ()
  ...
  (let* ((ext-version (my-xwiki-ext-jar-version))
         (src (my-xwiki-ext-jar ext-version))
         (dest (concat my-xwiki-data
                (url-hexify-string(concat "/extension/repository/eu.hbp.wiki:xwiki-ext-collab/" ext-version "/eu.hbp.wiki:xwiki-ext-collab-" ext-version) url-java-unreserved-chars)
                ".jar"))
          )
    (progn
        (shell-command (concat "cp " src " " dest))
        (xwiki-reload-tomcat)
        )))
